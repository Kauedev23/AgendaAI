...
import { Calendar, Scissors, LogOut, Clock, User as UserIcon } from "lucide-react";
...
const BarberDashboard = () => {
  ...
  const [barbeiro, setBarbeiro] = useState<(Tables<"barbeiros"> & { barbearias?: Tables<"barbearias"> }) | null>(null);
  const [agendamentos, setAgendamentos] = useState<Array<
    Tables<"agendamentos"> & {
      servicos?: { nome: string; preco: number; duracao: number };
      profiles?: { nome: string; telefone: string | null; email: string };
    }
  >>([]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      toast.error(`Erro ao carregar ${terminology.appointments.toLowerCase()}`);
    } finally {
      setLoading(false);
    }
  }, [navigate, terminology.appointments]);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                  ...
                  <p className="text-sm text-muted-foreground">{terminology.dashboardTitle}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="font-semibold">{profile?.nome}</p>
                <p className="text-sm text-muted-foreground">{terminology.professional}</p>
              </div>
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Aqui estão seus próximos {terminology.appointments.toLowerCase()}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <Card className="card-hover">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {agendamentosHoje.length === 1 ? terminology.appointment.toLowerCase() : terminology.appointments.toLowerCase()}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="card-hover">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {`${terminology.appointments} futuros`.toLowerCase()}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="card-hover">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {`${terminology.appointments} finalizados`.toLowerCase()}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Seus {terminology.appointments.toLowerCase()} agendados
            </CardDescription>
          </CardHeader>
          <CardContent>
            {agendamentos.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  {terminology.noAppointmentsFound}
                </p>
              </div>
            ) : (
